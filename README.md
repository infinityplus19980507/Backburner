...
Tasks are stored per user through `tasks.user_id`.

## Security

This version is set up so that:

- each `tasks` row belongs to a signed-in user
- database access is enforced with RLS using `auth.uid() = user_id`
- note images are stored in `note-images`
- note images are limited to the signed-in user's folder in Storage
