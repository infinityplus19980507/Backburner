...
const SUPABASE_IMAGE_BUCKET = "note-images";
const MINUTE_MAX = 60;
const MINUTE_STEP_HEIGHT = 52;
const SWIPE_THRESHOLD = 100;
const SUBTASK_LONG_PRESS_MS = 180;
const SUBTASK_DRAG_SLOP = 10;
...
let expandedSubtaskId = null;
let expandedAddNote = false;
...
function getStorageKey() {
  return currentUser ? `${STORAGE_KEY}:${currentUser.id}` : `${STORAGE_KEY}:anonymous`;
}
...
function readFilesAsDataUrls(fileList) {
  ...
  if (!supabaseClient) {
    setSyncStatus("Photos unavailable", "error");
    return Promise.resolve([]);
  }

  setSyncStatus("Uploading photos…", "saving");
  ...
  return Promise.all(
    files.map(async (file) => {
      try {
        ...
        const ownerPrefix = currentUser?.id || "anonymous";
        const path = `${ownerPrefix}/${Date.now()}-${crypto.randomUUID()}.${extension}`;
    ...
    setSyncStatus(uploaded.length ? "Photos uploaded" : "Photo upload failed", uploaded.length ? "saved" : "error");
    return uploaded;
  });
}
...
function setAuthMessage(message, tone = "idle") {
  if (!authMessage) {
    return;
  }

  authMessage.textContent = message;
  authMessage.dataset.state = tone;
}
...
function updateAuthView() {
  ...
  if (signedIn) {
    userChip.textContent = currentUser.email || "Signed in";
    setAuthMessage("Signed in.", "saved");
  } else {
    userChip.textContent = "";
    setSyncStatus("Sign in to sync", "idle");
    setAuthMessage("Your tasks and notes will stay tied to your account.", "idle");
    closePanels();
  }
}
...
async function fetchRemoteItems() {
  ...
  const { data, error } = await supabaseClient
    .from(SUPABASE_TABLE)
    .select("id, title, status, task_mode, subtasks, completed_subtasks, log, created_at, completed_at, sort_order")
  ...
  return (data || []).map((row) => ({
    ...
    taskMode: row.task_mode || "sequential",
...
async function persistRemoteItems(nextItems) {
  ...
  const rows = sanitizedItems.map((item) => ({
    ...
    title: item.title,
    status: item.status || "active",
    task_mode: item.taskMode || "sequential",
    subtasks: item.subtasks || [],
    completed_subtasks: item.completedSubtasks || [],
    log: item.log || [],
  ...
  if (rows.length) {
    const { error: upsertError } = await supabaseClient.from(SUPABASE_TABLE).upsert(rows, {
      onConflict: "id",
    });
...
async function handleAuthSubmit(event) {
  ...
  if (!supabaseClient) {
    setAuthMessage("Supabase is unavailable right now.", "error");
    return;
  }
  ...
  setAuthMessage("Sending magic link…", "saving");
  ...
  if (error) {
    ...
    setAuthMessage(error.message || "Could not send the magic link.", "error");
    return;
  }

  setAuthMessage("Check your email for the magic link.", "saved");
}
...
async function handleGoogleSignIn() {
  ...
  setAuthMessage("Opening Google sign-in…", "saving");
  ...
  if (error) {
    ...
    setAuthMessage(error.message || "Could not start Google sign-in.", "error");
  }
}
...
async function initializeAuth() {
  if (!supabaseClient) {
    setAuthMessage("Supabase client could not load.", "error");
    return;
  }
      ...
      <div class="selected-task-panel">
        ...
        <div class="note-box">
          ...
          <label class="field">
            ...
            <input id="completion-note-images" type="file" accept="image/*" multiple />
          </label>
          <div class="item-actions">
...
function markTaskDone(itemId) {
  ...
  const remainingSubtasks = [...item.subtasks];
  ...
  renderItemsBoard();
  renderItemDetail();
  return { item, completedCount: remainingSubtasks.length };
}

function completeSubtaskFromDetail(itemId, subtaskId) {
  expandedSubtaskId = null;
  const result = markSubtaskDone(itemId, subtaskId);
  return result;
}

function undoSubtaskDone(itemId, subtaskId) {
  const item = items.find((entry) => entry.id === itemId);
  if (!item) {
    return null;
  }

  const index = item.completedSubtasks.findIndex((entry) => entry.id === subtaskId);
  if (index === -1) {
    return null;
  }

  const [subtask] = item.completedSubtasks.splice(index, 1);
  const restored = {
    ...subtask,
    done: false,
  };
  delete restored.completedAt;
  item.subtasks.unshift(restored);
  item.status = "active";
  item.completedAt = null;

  const logIndex = [...item.log]
    .map((entry, idx) => ({ entry, idx }))
    .reverse()
    .find(({ entry }) => entry.type === "completed_subtask" && entry.subtaskName === subtask.text)?.idx;

  if (logIndex !== undefined) {
    item.log.splice(logIndex, 1);
  }

  saveItems();
  renderItemsBoard();
  renderItemDetail();
  return { item, restoredSubtask: restored };
}
...
function playCelebrationSound() {
  ...
  const notes = [
    ...
    { frequency: 783.99, duration: 0.18, offset: 0.18 },
  ];
  ...
  notes.forEach((note) => {
    ...
    gain.gain.exponentialRampToValueAtTime(0.18, startAt + note.offset + 0.02);
...
function showTaskCompletionCelebration(result) {
  if (!result?.item) {
    return;
  }
  ...
  itemDetail.innerHTML = `
    <div class="celebration-screen celebration-screen-task">
      ...
      <div>
        <p class="celebration-line">You finished it! 🎉</p>
        <h3>${escapeHtml(result.item.title)}</h3>
        <p class="hero-copy">
          ${result.completedCount
            ? `Amazing work. ${result.completedCount} remaining step${result.completedCount === 1 ? "" : "s"} got wrapped up and this task is now complete.`
            : "Amazing work. That whole task is officially done and moved to Completed."}
        </p>
      </div>
    </div>
  `;
...
function renderItemListButton(item) {
  ...
  return `
    <button
      class="item-list-button ${item.id === selectedItemId ? "active" : ""} ${isCompleted ? "completed-card" : ""}"
      type="button"
      data-action="open-item"
      data-item-id="${item.id}"
    >
      ${isCompleted ? '<div class="completed-card-burst" aria-hidden="true">🎉 🥳 ✨ 🎊 🎉</div>' : ""}
    ...
    <div class="detail-grid">
      <div class="card-heading compact">
        ...
        <div class="item-actions">
          ${
            item.status !== "completed"
              ? `<button class="ghost-button" type="button" data-action="complete-task" data-item-id="${item.id}">
                  Finish task
                </button>`
              : ""
          }
      ...
      <div class="soft-card">
        <div class="card-heading compact">
          <div>
            ...
            <p class="meta-copy">On phone, press and hold the grip to move a subtask around.</p>
          </div>
          ...
          ${item.subtasks
            .map(
              (subtask) => {
                ...
                return `
                <div class="subtask-editor-row" data-item-id="${item.id}" data-subtask-id="${subtask.id}">
                  <button
                    class="subtask-grip"
                    type="button"
                    data-subtask-grip
                    aria-label="Press and hold to move subtask"
                    ${item.status === "completed" ? "disabled" : ""}
                  >
                    ≡
                  </button>
                  <input
                    ...
                    ${item.status === "completed" ? "readonly" : ""}
                  />
                  <select class="subtask-desktop-control" data-field="subtask-minutes" data-item-id="${item.id}" data-subtask-id="${subtask.id}" ${
                    item.status === "completed" ? "disabled" : ""
                  }>
                  ...
                  <button
                    class="icon-button subtask-desktop-control"
                    type="button"
                    data-action="complete-subtask"
                    data-item-id="${item.id}"
                    data-subtask-id="${subtask.id}"
                  ...
                  <button
                    class="subtask-mobile-open"
                    type="button"
                    data-action="toggle-subtask"
                    data-subtask-id="${subtask.id}"
                    aria-label="Open subtask options"
                  >
                    ${isExpanded ? "Close" : "Edit"}
                  </button>
                  <div class="subtask-mobile-meta ${isExpanded ? "expanded" : ""}">
                    <label class="field">
                      <span>Time</span>
      ...
      ${
        item.completedSubtasks.length
          ? `
            <div class="soft-card">
              <div class="card-heading compact">
                <div>
                  <p class="section-label">Completed</p>
                  <h3>Reverse or redo a step</h3>
                </div>
              </div>
              <div class="detail-grid">
                ${[...item.completedSubtasks]
                  .sort((a, b) => (b.completedAt || 0) - (a.completedAt || 0))
                  .map(
                    (subtask) => `
                      <div class="completed-subtask-row soft-card">
                        <div>
                          <p class="log-title">✅ ${escapeHtml(subtask.text)}</p>
                          <p class="log-date">${subtask.completedAt ? formatDate(subtask.completedAt) : ""}</p>
                        </div>
                        <button
                          class="ghost-button"
                          type="button"
                          data-action="undo-subtask"
                          data-item-id="${item.id}"
                          data-subtask-id="${subtask.id}"
                        >
                          Undo
      ...
      <div class="soft-card">
        ...
        <div class="detail-grid">
          <button
            class="note-compose-toggle ghost-button"
            type="button"
            data-action="toggle-add-note"
          >
            ${expandedAddNote ? "Close note" : "Add note"}
          </button>
          <div class="note-compose-panel ${expandedAddNote ? "expanded" : ""}">
            <label class="field">
              <span>Add a title if you want</span>
            ...
            <label class="field">
              <span>Add photos</span>
              <input id="detail-note-images" type="file" accept="image/*" multiple />
            </label>
...
function renderLogEntry(itemId, entry) {
  ...
  const isExpanded = expandedNoteId === entry.id || editingNoteId === entry.id;

  if (editingNoteId === entry.id) {
    return `
      <div class="note-editor-row note-editor-row-editing soft-card">
        <div class="detail-grid">
          <label class="field">
            <span>Title</span>
            <input
          ...
          <label class="field">
            <span>Note</span>
            <textarea id="edit-note-text" rows="5" placeholder="Update your note">${escapeHtml(entry.text || "")}</textarea>
          </label>
          <div class="item-actions">
            <button
              class="ghost-button"
              type="button"
              data-action="cancel-edit-note"
              data-note-id="${entry.id}"
            >
              Cancel
            </button>
            <button
              class="primary-button secondary-tone"
              type="button"
              data-action="save-edit-note"
              data-item-id="${itemId}"
              data-note-id="${entry.id}"
            >
              Save changes
  ...
  if (!isExpanded) {
    const imageCount = entry.images?.length ? `<span class="note-preview-chip">${entry.images.length} photo${entry.images.length > 1 ? "s" : ""}</span>` : "";
    return `
      <button
        class="note-summary-button soft-card"
        type="button"
        data-action="toggle-note"
        data-note-id="${entry.id}"
      >
        <div>
          <p class="log-title">${escapeHtml(entry.title || "Note")}</p>
          <p class="log-date">${formatDate(entry.createdAt)}</p>
          <p class="note-preview-text">${escapeHtml((entry.text || "").slice(0, 120))}${entry.text?.length > 120 ? "..." : ""}</p>
        </div>
        <div class="note-preview-meta">
          ${imageCount}
          <span class="note-preview-open">Open</span>
        </div>
      </button>
    `;
  }
...
function clearSubtaskDropTargets() {
  itemDetail.querySelectorAll(".subtask-editor-row").forEach((entry) => {
    entry.classList.remove("drop-target");
  });
}

function cleanupSubtaskTouchDrag() {
  if (subtaskTouchDrag.timerId) {
    clearTimeout(subtaskTouchDrag.timerId);
  }

  if (subtaskTouchDrag.row) {
    subtaskTouchDrag.row.classList.remove("touch-dragging");
    subtaskTouchDrag.row.style.transform = "";
    subtaskTouchDrag.row.style.zIndex = "";
  }
...
function findSubtaskDropTarget(clientY, draggedRow) {
  const rows = Array.from(itemDetail.querySelectorAll(".subtask-editor-row"));
  let bestRow = null;
...
function toggleNoteExpansion(noteId) {
  ...
  if (editingNoteId && editingNoteId !== expandedNoteId) {
    editingNoteId = null;
  }
  renderItemDetail();
}

function toggleSubtaskExpansion(subtaskId) {
  expandedSubtaskId = expandedSubtaskId === subtaskId ? null : subtaskId;
  renderItemDetail();
}

function toggleAddNoteComposer(forceOpen = null) {
  expandedAddNote = forceOpen === null ? !expandedAddNote : Boolean(forceOpen);
  renderItemDetail();
}

function updateNote(itemId, noteId, title, text) {
  const item = items.find((entry) => entry.id === itemId);
  const note = item?.log.find((entry) => entry.id === noteId && entry.type === "note");
  if (!note) {
    return;
  }
...
quickWinResult.addEventListener("click", (event) => {
  ...
  if (action === "save-note" && activeQuickTask) {
    ...
    const imageField = document.getElementById("completion-note-images");
    const note = noteField.value.trim();
    if (!note) {
      noteField.focus();
      return;
    }
    readFilesAsDataUrls(imageField?.files).then((images) => {
...
itemDetail.addEventListener("click", (event) => {
  ...
  if (action === "complete-task") {
    ...
    editingNoteId = null;
    expandedNoteId = null;
    const result = markTaskDone(itemId);
    showTaskCompletionCelebration(result);
  }
  ...
  if (action === "complete-subtask") {
    completeSubtaskFromDetail(itemId, subtaskId);
  }

  if (action === "undo-subtask") {
    undoSubtaskDone(itemId, subtaskId);
  }
  ...
  if (action === "save-edit-note") {
    const titleField = document.getElementById("edit-note-title");
    const textField = document.getElementById("edit-note-text");
    ...
    updateNote(itemId, noteId, titleField?.value || "", nextText);
  }
...
itemDetail.addEventListener("pointerdown", (event) => {
  ...
  const grip = event.target.closest("[data-subtask-grip]");
  if (!grip) {
    return;
  }

  cleanupSubtaskTouchDrag();
  ...
  subtaskTouchDrag = {
    ...
    timerId: window.setTimeout(() => {
      subtaskTouchDrag.active = true;
      subtaskTouchDrag.row = row;
      row.classList.add("touch-dragging");
      row.style.zIndex = "4";
      try {
        row.setPointerCapture(event.pointerId);
      } catch (_error) {
        // Ignore capture issues on browsers that are picky here.
      }
    }, SUBTASK_LONG_PRESS_MS),
    itemId,
    subtaskId,
    row,
    startY: event.clientY,
...
itemDetail.addEventListener("pointermove", (event) => {
  if (event.pointerType !== "touch" || event.pointerId !== subtaskTouchDrag.pointerId) {
    return;
  }

  const deltaY = event.clientY - subtaskTouchDrag.startY;

  if (!subtaskTouchDrag.active) {
    if (Math.abs(deltaY) > SUBTASK_DRAG_SLOP && subtaskTouchDrag.timerId) {
      clearTimeout(subtaskTouchDrag.timerId);
      subtaskTouchDrag.timerId = null;
    }
    return;
  }

  event.preventDefault();
  subtaskTouchDrag.currentY = event.clientY;
  subtaskTouchDrag.row.style.transform = `translateY(${deltaY}px) scale(1.01)`;

  clearSubtaskDropTargets();
  const targetRow = findSubtaskDropTarget(event.clientY, subtaskTouchDrag.row);
  if (targetRow) {
    targetRow.classList.add("drop-target");
  }
});

function endSubtaskTouchDrag(event) {
  if (event.pointerType !== "touch" || event.pointerId !== subtaskTouchDrag.pointerId) {
    return;
  }

  const wasActive = subtaskTouchDrag.active;
  const itemId = subtaskTouchDrag.itemId;
  const subtaskId = subtaskTouchDrag.subtaskId;
  const draggedRow = subtaskTouchDrag.row;
  const targetRow = wasActive ? findSubtaskDropTarget(event.clientY, draggedRow) : null;
  const targetSubtaskId = targetRow?.getAttribute("data-subtask-id");

  cleanupSubtaskTouchDrag();

  if (!wasActive || !itemId || !subtaskId || !targetSubtaskId) {
    return;
  }
...
itemDetail.addEventListener("pointerup", endSubtaskTouchDrag);
itemDetail.addEventListener("pointercancel", endSubtaskTouchDrag);
